...
pingDB (ParseUserId u) = do
    let uid = extractFromSnowflake u
```
...
myPingAction = return $ (messageDetailsBasic "pong") { messageDetailsComponents = Just [cps] }
    where cps = ComponentActionRowButton [mkButton "Ping!" "myping pingbutton"]
```
...
myPingButtonAction i inter = do
    ...
    where cps = ComponentActionRowButton [mkButton "Ping!" ("myping pingbutton " <> pack (show (i + 1)))]
```
...
myPingAction :: DatabaseDiscord MessageDetails
myPingAction = return $ (messageDetailsBasic "pong") { messageDetailsComponents = Just [cps] }
    where cps = ComponentActionRowButton [mkButton "Ping!" "myping pingbutton 0"]
