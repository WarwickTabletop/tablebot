...
-- distribution.
chooseOne :: (MonadRandom m, MonadException m) => [a] -> m a
chooseOne [] = throw $ RandomException "Cannot choose from empty list."
chooseOne xs = (xs !!) <$> getRandomR (0, length xs - 1 :: Int)
...
chooseOneWithDefault :: (MonadRandom m, MonadException m) => a -> [a] -> m a
...
chooseOneWeighted :: (MonadRandom m, MonadException m) => (a -> Int) -> [a] -> m a
...
chooseOneWeighted weight xs
  ...
  | otherwise =
    fst . fromJust . (\i -> find ((> i) . snd) (zip xs' $ scanl1 (+) $ weight <$> xs')) <$> getRandomR (0, totalWeight - 1)
  where
...
chooseOneWeightedWithDefault :: (MonadRandom m, MonadException m) => a -> (a -> Int) -> [a] -> m a
